...
public class Message {
    ...
    public static Message fromJson(String json) {
        ...
        return new Message(array.values().stream().map(e -> mapper.treeToValue(e, Segment.class)).toArray(Segment[]::new));
    }

    public static Message fromCQ(String cq) {
        String[] cqSegments = MessageUtil.splitToArray(cq);
        return new Message(Arrays.stream(cqSegments).map(cqDeserializer::deserialize).toArray(Segment[]::new));
    }
